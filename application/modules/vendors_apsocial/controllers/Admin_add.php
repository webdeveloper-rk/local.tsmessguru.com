...
class Admin_add extends MX_Controller {
    ...
    function __construct() {
		...
		if($this->uri->segment(2) !="login") { 
					...
					if($this->session->userdata("user_role") != "subadmin" && $this->session->userdata("school_code") != "10100" )
					{
						redirect("admin/login");
							die;
					}
	 ...
	 function entryform($account_type='',$school_id=0)
	{
        ...
        $data["view_file"] = "vendor_form_admin";
        echo Modules::run("template/admin", $data);
         
	}
	public function ajax_add_vendor($account_type='',$school_id)
	{
		$allowed_account_types  = array("firm","individual");
		if(!in_array($account_type,$allowed_account_types))
		{
