...
class Monthly_consolidated_tsmess extends MX_Controller {
...
(SELECT school_id  , sum(amount) as deduction_amount FROM    dietpic_cheker  where   DATE_FORMAT( entry_date,  '%m-%Y' ) = ?  and  min_20='no' group by  school_id)
 dm on sc.school_id = dm.school_id  ",array($month_year));			
			...
			sum(cat1_attendence ) as group1_regular_attendence,
			sum(cat1_guest_attendence  ) as group1_guest_attendence,
			 
			sum(cat2_attendence ) as group2_regular_attendence,
			sum(cat2_guest_attendence  ) as group2_guest_attendence,

			sum(cat3_attendence ) as group3_regulur_attendence,
			sum(cat3_guest_attendence  ) as group3_guest_attendence 
		 ...
		 // echo $this->db->last_query();die;
			foreach($attendence_rs->result() as $atrow)
			{
				...
				$report_data[$atrow->school_id]['group1_regular_attendence'] = $atrow->group1_regular_attendence;
				$report_data[$atrow->school_id]['group1_guest_attendence'] = $atrow->group1_guest_attendence;
				
				$report_data[$atrow->school_id]['group2_regular_attendence'] = $atrow->group2_regular_attendence;
				$report_data[$atrow->school_id]['group2_guest_attendence'] = $atrow->group2_guest_attendence;
				
				
				$report_data[$atrow->school_id]['group3_regulur_attendence'] = $atrow->group3_regulur_attendence;
				$report_data[$atrow->school_id]['group3_guest_attendence'] = $atrow->group3_guest_attendence; 
			}
		...
		$data["module"] = "monthly_consolidated_tsmess";
		  ...
		  $months = array("01"=>"January","02"=>"February","03"=>"March","04"=>"April","05"=>"May",
				...
				$this->excel->getActiveSheet()->setCellValue('Q3', 'Remaining Amount');
					...
					if($this->config->item("deductions_enabled") == true && $this->session->userdata("is_dco") ==0 )
					{
						...
						$this->excel->getActiveSheet()->setCellValue('U3', 'Cat 1 Regular Attendance  ');
						$this->excel->getActiveSheet()->setCellValue('V3', 'Cat 1 Guest Attendance ');
						$this->excel->getActiveSheet()->setCellValue('W3', 'Cat 2 Regular Attendance ');
						$this->excel->getActiveSheet()->setCellValue('X3', 'Cat 2 Guest Attendance ');
						$this->excel->getActiveSheet()->setCellValue('Y3', 'Cat 3 Regular Attendance ');
						$this->excel->getActiveSheet()->setCellValue('z3', 'Cat 3 Regular Attendance ');
				...
				foreach($school_attendence as $school_code=>$school_data){ 
					...
					if($this->config->item("deductions_enabled") == true && $this->session->userdata("is_dco") ==0)
					{
						...
						$this->excel->getActiveSheet()->setCellValue('U'.$i,  $school_code['group1_regular_attendence']);
						$this->excel->getActiveSheet()->setCellValue('V'.$i,  $school_code['group1_guest_attendence']);
						$this->excel->getActiveSheet()->setCellValue('W'.$i,  $school_code['group2_regular_attendence']);
						$this->excel->getActiveSheet()->setCellValue('X'.$i,  $school_code['group2_guest_attendence']);
						$this->excel->getActiveSheet()->setCellValue('Y'.$i,  $school_code['group3_regular_attendence']);
						$this->excel->getActiveSheet()->setCellValue('Z'.$i,  $school_code['group3_guest_attendence']);
