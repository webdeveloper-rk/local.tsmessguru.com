...
class Monthly_consolidated_march_2020 extends MX_Controller {
	...
	$months = array("01"=>"January","02"=>"February","03"=>"March","04"=>"April","05"=>"May",
		...
		$to_date = '2020-03-15';	
		 $bill_no_of_days = $this->db->query("SELECT DATEDIFF(?,?) as days",array($from_date,$to_date))->row()->days;
			...
			FROM   school_attendence  where   entry_date between ? and ? group by school_id  ;";
			 
			
			$attendence_rs =  $this->db->query($attendece_allowed_table_sql,array($from_date,$to_date));
		 // echo $this->db->last_query();die;
			foreach($attendence_rs->result() as $atrow)
			{
				...
				foreach($price_rs->result() as $stu_price){
					 ...
					 $stu_price_amount = 	 ($stu_price->amount/ $days_count) * $bill_no_of_days; 
					$school_prices[$stu_price->category][$stu_price->group_code] = $stu_price_amount;
				}
				
				
				
				$extra_amounts_rs = $this->db->query("select school_id,sum(class_10th_attendance+iner_attendance)*extra_amount as extra_amount
				from extra_bill where   entry_date between ? and ? group by school_id ",array($report_date,$to_date));
				$extra_amounts = array();
				...
				foreach($schools_rs->result() as $school_item){
					...
					$report_data[$school_item->school_id]['group1_amount_perday'] = $school_prices[$school_item->amount_category]['gp_5_7']/$bill_no_of_days ;
					$report_data[$school_item->school_id]['group2_amount_perday'] = $school_prices[$school_item->amount_category]['gp_8_10']/$bill_no_of_days ;
					$report_data[$school_item->school_id]['group3_amount_perday'] = $school_prices[$school_item->amount_category]['gp_inter'] /$bill_no_of_days;
			...
			$balance_sheet_table = $this->common_model->get_stock_entry_table($report_date);
			  ...
			  $consumed_table_sql  = "   SELECT  school_id as school_id,
						...
						as total_consumed from $balance_sheet_table where  entry_date between ? and ? group by school_id ";
				

			//echo $consumed_table_sql;die;
			$consumed_rs = $this->db->query($consumed_table_sql,array($from_date,$to_date));
			//echo $this->db->last_query();die;
		...
		$data["module"] = "monthly_consolidated_march_2020";
	...
	public function attendence_consumed_report($school_attendence,$report_date_frmted,$report_date_toted,$schools_info=array())
    {
		...
		$extra_data = $school_attendence['extra'];
		  ...
		  $months = array("01"=>"January","02"=>"February","03"=>"March","04"=>"April","05"=>"May",
                ...
                //name the worksheet
                $this->excel->getActiveSheet()->setTitle('Attendance & Consumption Report March 1st half');
