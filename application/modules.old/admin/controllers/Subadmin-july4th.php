...
//print_a($uploads);
		...
		$data["report_date"] = $report_date_formated;
					...
					if($uid <10)
					{
							$sql = "SELECT * from schools where name not like 'coll%' ";
							$rs = $prs  = $this->db->query($sql);
					}
					else
					{
										$sql = "SELECT * from schools where district_id='$district_id' and name not like 'coll%'";
										$rs = $prs  = $this->db->query($sql);
					}
	...
	function reports($type='district')
	{
		...
		if($type=="district"){
					...
					else
					{
						...
						// print_a( $school_info);
						$data["school_rs"] = $this->db->query("select s.*,d.name as district_name  from schools s inner join districts d on s.district_id = d.district_id and s.district_id='$district_id' and s.name not like '%COLLECTOR%'");
					}
