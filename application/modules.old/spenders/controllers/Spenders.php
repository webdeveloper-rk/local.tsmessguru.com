	...
	function amountwise_report($encoded_date=null)
	{
		...
		$sql = "SELECT 
						sc.school_id,sc.name,sc.school_code ,entry_date, 
						 ...
						 FROM $balance_sheet_table  bs inner join items it on it.item_id  = bs.item_id  
		...
		else
		{
		  ...
		  $balance_sheet_table = $this->common_model->get_stock_entry_table($report_date);
			 ...
			 (session_1_qty+ session_2_qty+session_3_qty+session_4_qty )  as total_qty,
				...
				(session_4_qty* session_4_price )  ) as total_price 
			from 
			 $balance_sheet_table bs inner join items it on it.item_id = bs.item_id and bs.entry_date=?  and bs.school_id=? order by total_price desc ";
